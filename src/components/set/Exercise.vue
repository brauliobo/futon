    ...
    div(v-if="usesFractionInput && !isReadOnly" class="flex items-center gap-3")
      ...
      div(class="relative ml-auto")
        FractionAnswerInput(
          ...
          :aria-label="`${$t('enterAnswer') || 'Answer'} ${exerciseNumber}`"
          :clear-label="$t('clear') || 'Clear'"
          :whole-label="$t('fractionWhole') || 'Whole'"
          :numerator-label="$t('fractionNumerator') || 'Numerator'"
          :denominator-label="$t('fractionDenominator') || 'Denominator'"
          @submit="handleSubmit"
