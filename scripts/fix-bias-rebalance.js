...
async function main() {
  ...
  for (const f of files) {
    ...
    // Rotate every other biased exercise for each needFix choice-count.
    let rotatedHere = 0;
    const seen = new Map(); // ch-count → count of biased-at-majority seen
    for (const p of s.pages || []) for (const e of p.exercises || []) {
      const q = String(e.question);
      const m = q.match(CHOICE_RE);
      if (!m) continue;
      ...
      if (idx !== majority) continue; // only rotate exercises at the biased position
      const k = `${n}`;
      seen.set(k, (seen.get(k) || 0) + 1);
      // Rotate every other one (1st, 3rd, 5th…) so we don't over-rotate.
      if (seen.get(k) % 2 !== 1) continue;
