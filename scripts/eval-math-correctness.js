...
function verify(question, answer, type) {
  ...
  if (type === 'factoring' || type === 'algebraic_expression' || type === 'algebra') {
    ...
    const lvars = [...new Set([...qExpr.matchAll(/\b([a-z])\b/g)].map(m => m[1]))]
      .filter(v => v !== 'e' && v !== 'i');
    const result = probeEquivalent(qExpr, a, lvars.length ? lvars : ['x']);
